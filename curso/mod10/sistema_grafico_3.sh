...
#Função Remover com interface gráfica
...
RemoveUsuario () {
    ...
    local usuarios=$(egrep "^#|^$" -v "$ARQ_BD" | sort -h | cut -d $SEP -f 1,2 | sed 's/:/ "/;s/$/"/')
    local id_rm=$(eval dialog --stdout --menu \"Escolha um usuário:\" 0 0 0 $usuarios)
